...
export async function sendLeadWelcome({
  ...
  if (!resend) return;
  const iniciales = comercial.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
  try {
      ...
      html: emailWrapper(`
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;margin-bottom:24px;">
          <tr><td style="padding:16px 18px;">
            ...
            ${inmuebleUrl
              ? `<a href="${inmuebleUrl}" style="font-size:14px;font-weight:600;color:#0f172a;text-decoration:none;">${inmueble}</a>`
